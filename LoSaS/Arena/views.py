...
def enemy_select(request):
    ...
    enemies = Character.objects.exclude(user=request.user, level__lt=min_level, level__gt=max_level)

    # Calculate the Life value for each opponent and pass it to the template
    for enemy in enemies:
        enemy.life = int(
            enemy.hp * ((1 + enemy.constitution * 0.1) * (1 + enemy.wisdom * 0.1)) / 10)

    return render(request, 'Arena/arena_select.html', {'enemies': enemies})
...
def fight(request):
    ...
    if selected_enemy_id:
        ...
        # If there is no selected opponent, draw a new one
        enemies = choice(Character.objects.filter(level__gte=min_enemy_level, level__lte=max_enemy_level))
